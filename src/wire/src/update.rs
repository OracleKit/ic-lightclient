...
impl UpdatePayloadParser {
    pub fn new(data: Vec<u8>) -> Result<Self> {
        let updates: CanisterUpdates = serde_json::from_slice(data.as_slice())
    ...
    pub fn updates<T: DeserializeOwned>(&self, uid: u16) -> Result<Vec<T>> {
        let raw_updates = self.updates.updates.get(&uid)
            .ok_or(anyhow!("No updates for chain uid: {}", uid))?;

        let updates = raw_updates.updates.iter().map(|raw_update| {
...
impl UpdatePayloadMarshaller {
    pub fn new() -> Self {
        Self {
            updates: CanisterUpdates { version: 1, updates: HashMap::new() }
        }
    }

    pub fn updates<T: Serialize>(&mut self, uid: u16, updates: Vec<T>) -> Result<()> {
        let marshalled_updates = updates.into_iter().map(|update| {
            serde_json::to_vec(&update)
                .context("Failed to marshal chain update")
        }).collect::<Result<Vec<Vec<u8>>>>()?;

        self.updates.updates.insert(uid, ChainUpdates {
            version: 1,
            updates: marshalled_updates
        });
    ...
    pub fn build(&self) -> Result<Vec<u8>> {
        serde_json::to_vec(&self.updates)
            .context("Failed to marshal canister update")
    }
}